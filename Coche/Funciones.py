...
def send_request(file_list = [], 
                    ...
                    img_size = 640,
                    tracking = False):
    ''' Esta función envia la solicitud a la nube, agregando la imagen y el nombre del modelo. Además si se quiere hacer traqueo hay que indicarlo.'''
...
def results_to_json(results, model):
    ''' Convierte la salida del modelo yolo en una lista de listas de diccionarios'''
    return [
                [
                    {
                    "class": int(pred[5]),
...
def add_text_to_image(image, timer, datos, FPS):
    ''' Funcion que impllemeta la interfaz grafica de las metricas de velocidad del coche'''
    ...
    if len(datos) > 1:
        cv2.putText(overlay, "FPS: " + f'{FPS:.2f}', (text_x, text_y - text_height - 10), font, font_scale, (255, 20, 147), text_thickness, line_type)
        cv2.putText(overlay, "RTT: " + f'{timer*1000:.2f}' + "ms", (text_x, text_y), font, font_scale, (255, 20, 147), text_thickness, line_type)
        cv2.putText(overlay, "T. proc: " + datos[1] + "ms", (text_x, text_y + text_height + 10), font, font_scale, (255, 20, 147), text_thickness, line_type)
    else:
        cv2.putText(overlay, "FPS: " + f'{FPS:.2f}', (text_x, text_y - text_height), font, font_scale, (255, 20, 147), text_thickness, line_type)
        cv2.putText(overlay, "T. proc: " + f'{timer*1000:.2f}' + "ms", (text_x, text_y + text_height), font, font_scale, (255, 20, 147), text_thickness, line_type)
